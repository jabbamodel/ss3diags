...
SSplot_eqcurves = function(ss3rep,Fref=c("MSY","Btgt","SPR","F01")[2],msyline=NULL,cex=0.7){ 
  ...
  text(bmsy*0.85,ymsy*1.05,expression(B[MSY]),cex=cex+0.1)
  if(Fref!="MSY"){
  lines(c(btgt,btgt),c(0,ytgt),col=cols[3])
  text(btgt*1.05,ytgt*1.05,bquote("B"[.(trg)]),cex=cex+0.1)
  }
  ...
  text(fmsy*1.05,ymsy*1.05,expression(F[MSY]),cex=cex+0.1)
  if(Fref!="MSY"){
    lines(c(ftgt,ftgt),c(0,ytgt),col=cols[3])
    text(ftgt*0.95,ytgt*1.05,bquote("F"[.(trg)]),cex=cex+0.1)
  }
  if(!is.null(msyline)) abline(h=msyline*ymsy,lty=2,col=2)
