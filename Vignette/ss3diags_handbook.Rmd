...
author: "Hennig Winker (JRC-EC), Felipe Carvalho (NOAA), Massimiliano Cardinale (SLU) & Laurence Kell (Sea++)"
...
This vignette explains functions for applying advanced model diagnostics for Stock Synthesis model with the `ss3diags` R package for model. The `ss3diag` R package accompanies the paper "A cookbook for using model diagnostics in integrated stock assessments"  by [Carvalho, Winker et al. (2021)](https://www.sciencedirect.com/science/article/pii/S0165783621000874). The `ss3diags` package builds on R package `r4ss` (Taylor et al. 2021), which is designed to support the use of the Stock Synthesis software (Methot and Wetzel, 2013).
...
The package contains two examples of Stock Synthesis assessments as presented in [Carvalho, Winker et al. (2021)](https://www.sciencedirect.com/science/article/pii/S0165783621000874). 
...
The type of reference points dependents on settings in the `starter.ss` file that determine the derived quanties `Bratio` and `Fvalue`. 

In the first example, we consider the `ss3sma` Stock Synthesis model, which was run with `starter.ss` settings that are commonly to produce target Kobe plot estimates of $SSB/SSB_{MSY}$ and $F/F_{MSY}$ in tRFMO assessments: 
...
We provide the the function `SSsettingsBratioF(ss3sma)` to the `starter.ss` settings:
...
This function is also inbuilt in the `SSdeltaMVLN()` to prevent misleading results. To generate a joint MVLN of $SSB/SSB_{MSY}$ and $F/F_{MSY}$, we can use the default options

```{r fig10, fig.width=4,fig.cap="Kobe phase plots showing MVLN Kobe probability distributions of $SSB/SSB_{MSY}$ and $F/F_{MSY}$ for North Atlantic shortfin mako Stock model."}
...
The `SSdeltaMVLN()` output include the maximum the maximum likelihood estimates (mles) and the MVLN monte-Carlo distributions `$kb` of $SSB/SSB_{MSY}$, $F/F_{MSY}$ and $F$. Note the additional quanties $SSB$ and $Rec$ are generated independely from lognormal distributions for practical reasons. These can be plotted by

```{r fig11, fig.height=6.5,fig.cap="Distributions for $SSB/SSB_{MSY}$ $F/F_{MSY}$, $SSB$, Recruitment and Catch trajectories for the North Atlantic shortfin mako SS3 model"}
sspar(mfrow=c(3,2),plot.cex = 0.7)
SSplotEnsemble(mvln$kb,ylabs=mvln$labels,add=T,verbose=F) 
```

The `SSdeltaMVLN()` provides the option to set alternative `Fref` values, but this is only possible for the recommended `starter.ss` option 0 for `F_report_basis`. For option 2, `SSdeltaMVLN()` get an error if `Fref` where to be changed.

```{r error=TRUE}
mvln = SSdeltaMVLN(ss3sma,run="SMA", Fref="Btgt") 
```
...
The management quantaties are in this case $SSB/SSB_{40}$ and $F/F_{spr40}$, where the target of 40\% is specified in the `forecast.ss` file.
...
```{r fig14, fig.height=6.5,fig.cap="Distributions for $SSB/SSB_{40}$ $F/F_{SPR40}$, $SSB$, Recruitment and Catch trajectories for the Pacific Hake SS3 model"} 
sspar(mfrow=c(3,2),plot.cex = 0.7)
SSplotEnsemble(mvln$kb,ylabs=mvln$labels,add=T,verbose=F) 
```
...
It is important to note that MVLN approximation differs notably from MCMC posterior for this model as has been documented in [Stewart et al. (2013)](https://www.sciencedirect.com/science/article/abs/pii/S0165783612002081) and [Taylor et al. (2021)](https://www.sciencedirect.com/science/article/abs/pii/S0165783621000527). Such differences may be more likely to occur in cases where key parameters such as steepness $h$ or natural $M$ are estimated using informative priors, which can result in left skewed (non-lognormal) distributions of the benchmarks $F_{ref}$ and $B_{ref}$. 

In other instances, mismatches between the`SSdeltaMVLN` and MCMC may also be caused by poor performance of the latter due to poor regularization [(Monnahan et al., 2019)](https://academic.oup.com/icesjms/article/76/6/1477/5475859?login=true).

To facilitate comparison between the `SSdeltaMLVN()` and MCMC outputs, we provide the function `SSdiagsMCMC()`, which we illustrate on the example of the Stock Synthesis model for the ICES Gulf of Bothian Herring stock [(ICES, 2021)](https://www.ices.dk/sites/pub/Publication%20Reports/Stock%20Annexes/2021/her.27.3031_SA.pdf). 
...
The options and output of `SSdiagsMCMC()` is largely identical to `SSdeltaMVLN`. In this case, the `starter.ss` is the same as for `ss3phk`, only the we use Fref = "Btgt" for illustration with $SB_{50}$ and $F_{SB50}$ for this illustration.
...
Comparing detla-MVLN with MCMC simply requires combining the `$kb` outputs by, e.g.,


```{r fig15, fig.height=3,fig.width=3.8,fig.cap="Kobe phase plot comparing MVLN and MCMC posterior distributions of $SSB/SSB_{50}$ and $F/F_{50}$  for the ICES Gulf of Bothia Herring SS3 model"} 
...
```{r fig16, fig.height=4.5,fig.cap="Comparison of MVLN and MCMC posterios for $SSB/SSB_{40}$ $F/F_{SB40}$, $SSB$ for the ICES Gulf of Bothia Herring SS3 model"} 

sspar(mfrow=c(2,2),plot.cex = 0.7)
SSplotEnsemble(rbind(mvln$kb,mcmc$kb),ylabs=mvln$labels,
add=T,subplots = c("stock", "harvest", "SSB", "F"),
verbose=F)
