...
#' @param pars quantaties to extract from Hessian
...
ssdeltaMVLN = function(ss3rep,pars=c('Bratio','F'),years=NULL,mc=5000,weight=1,run="MVLN",plot=TRUE,
  ...
  # Get years
  allyrs = unique(as.numeric(gsub(paste0(pars[2],"_"),"",quants$Label[grep(paste0(pars[2],"_"), quants$Label)])))[-1]
  ...
  for(yi in 1:length(yrs)){ 
    yr = yrs[yi]
    x <- cv[cv$label.j %in% paste0(pars[2],"_",c(yr-1,yr,yr+1)) & cv$label.i %in% paste0(pars[1],"_",c(yr-1,yr,yr+1)),]
    y = hat[ylabel %in% paste0(pars,"_",yr),] # old version Label not LABEL
  ...
  if(fb==1){
    kb[,3] = kb[,3]/hat[hat$Label=="Fstd_MSY",2]  
    mle[,3] = mle[,3]/hat[hat$Label=="Fstd_MSY",2]
  }
  ...
  if(plot==TRUE){
    if(is.null(xmax)) xmax= max(c(2,kb[kb$year==max(kb$year),2],mle[,2]))
    if(is.null(ymax)) ymax = max(c(2,kb[kb$year==max(kb$year),3],mle[,3]))
    stock = kb[kb$year==max(kb$year),2]
    harvest = kb[kb$year==max(kb$year),3]
    plot(kb[kb$year==max(kb$year),2:3]  ,type="n",ylab=ylab,xlab=xlab,xlim=c(0,xmax),ylim=c(0,ymax),yaxs="i",xaxs="i")  
    ...
    points(kb[kb$year==max(kb$year),2:3],pch=21,bg="grey",cex=0.7)
    if(addtrj & nrow(mle)>3){
      lines(mle[,2:3])  
      points(mle[,2:3],pch=21,cex=0.9,bg="white")
      points(mle[1,2:3],pch=24,bg="white",cex=1.7)
      } 
    points(mle[nrow(mle),2:3],pch=21,bg="white",cex=2)
  ...
  return(list(kb=kb,mle=mle,labels=c(xlab,ylab)))
  }
